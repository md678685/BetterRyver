const React = require("react");
const jsx = require("jsx-transform");

module.exports = class BetterRyverUI {
    constructor() {
        this.AlertBar = React.createClass({
            render: function() {
                return jsx.fromFile("jsx/alertbar.jsx");
            }
        });
    }

    popup(message) {
        /*let div = document.createElement("div");
        div.innerHTML = message;
        div.style.padding = "10px";
        div.style.fontFamily = "'Open Sans', sans-serif";*/
        let div = React.createElement(this.AlertBar, {message: message});
        return div;
    }

    error(message) {
        let div = this.popup(message);
        /*div.style.backgroundColor = "#ffc3c3";
        div.style.borderColor = "#dd4646";
        div.style.borderBottomWidth = "2px";*/
        document.getElementsByTagName("body")[0].insertBefore(div, document.getElementById("appMountPoint"));
    }
}